...
def setup_components(shared_data, system_boot_ms, server_ip, server_udp_port):
    ...
    heartbeat_msg = sim_conn.wait_heartbeat()
    # wait_heartbeat() doesn't reliably populate target_component from the received message
    # (it stayed 0 while the vehicle's real component was 200) - set both explicitly.
    sim_conn.target_system = heartbeat_msg.get_srcSystem()
    sim_conn.target_component = heartbeat_msg.get_srcComponent()
    print(f"Connected to system: {sim_conn.target_system}, component: {sim_conn.target_component}", flush=True)
